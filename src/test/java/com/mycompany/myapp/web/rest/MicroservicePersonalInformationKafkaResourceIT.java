...
class MicroservicePersonalInformationKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/microservice-personal-information-kafka/publish?message=value-produce").with(csrf()))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/microservice-personal-information-kafka/register"))
            .andExpect(status().isOk())
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/microservice-personal-information-kafka/unregister"));
                return;
            }
        }
